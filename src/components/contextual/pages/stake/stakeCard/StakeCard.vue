...
const { connectToAppNetwork } = useStakeWeb3();
...
function updateNetWorkInputFrom(chainId) {
  let networkChoose = NETWORKS.find(item => item.chain_id_decimals === chainId);
  if (networkChoose) {
    ...
    >
      ...
      <div class="bridge-container">
        ...
        <div v-if="isWalletReady" class="bridge-actions">
          <BalBtn
            v-if="estimateInfo?.err?.code === '9999'"
            :label="$t('Switch network')"
            :loading="isLoading"
            classCustom="pink-white-shadow"
            block
            @click.prevent="handleNetworkChange(DEFAULT_CHAIN_ID)"
          />
          <BalBtn
            v-else
            :disabled="
