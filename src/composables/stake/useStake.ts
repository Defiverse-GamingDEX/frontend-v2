...
import { OASYS_TESTNET_NETWORK } from '@/constants/stake/oasys-testnet-network';
import { default as ERC20ABI } from '@/lib/abi/ERC20.json';
import { default as StakeABI } from '@/lib/abi/stake/Stake.json';
import { bnum } from '@/lib/utils';
import { Contract } from '@ethersproject/contracts';
import { JsonRpcProvider } from '@ethersproject/providers';
import BigNumber from 'bignumber.js';
import { ethers } from 'ethers';
import stakeAPI from './stake.api.js';
import stakeService from './stake.services.js';
// function from stakeAPI - START
async function getTransferConfigs() {
  try {
    const rs = await stakeAPI.getTransferConfigs();
...
async function getEstimateAmt(inputFrom, inputTo, account, slippage) {
  try {
    ...
    const rs = await stakeAPI.getEstimateAmt(params);
...
async function getTransferStatus(transfer_id) {
  try {
    const rs = await stakeAPI.getTransferStatus(transfer_id);
...
async function getTransferHistory(account, paging) {
  try {
    ...
    const rs = await stakeAPI.getTransferHistory(params);
...
function getChainName(chainId) {
  return NETWORKS.find(item => item.chain_id_decimals === chainId)?.name;
}
function getChain(chainId) {
  return NETWORKS.find(item => item.chain_id_decimals === chainId);
}
...
async function bridgeSend(
  ...
  try {
    ...
    if (chainTransfer.type === 'L1toL1') {
      //tx = await stakeService.bridgeSendNative(params);
      tx = await stakeService.bridgeSend(params);
